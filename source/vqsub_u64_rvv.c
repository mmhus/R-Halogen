...
/**
 * @brief Performs an element-wise unsigned saturating subtraction of two 64-bit vectors of 64-bit unsigned integers.
 * 
 * This function performs an element-wise subtraction of two 64-bit unsigned integer vectors (`a` and `b`), 
 * each containing one elements. If the result of the subtraction exceeds the range of an 64-bit unsigned integer 
 * (0 to 18446744073709551615), the result is saturated to the nearest boundary (either 0 or 18446744073709551615). It 
 * utilizes the RISC-V Vector Extension to carry out the operation, making it analogous to the ARM Neon function `vqsub_u64`. 
...
uint64x1_t vqsub_u64_rvv(uint64x1_t a, uint64x1_t b) {
    return __riscv_vssubu_vv_u64m1(a, b, VLEN_1);
}
