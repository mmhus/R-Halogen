...
/**
 * @brief Performs an element-wise signed saturating addition of two 128-bit vectors of 64-bit signed integers.
 * 
 * This function performs an element-wise addition of two 64-bit signed integer vectors (`a` and `b`), 
 * each containing two elements. If the result of the addition exceeds the range of an 64-bit signed integer 
 ...
 * It utilizes the RISC-V Vector Extension to carry out the operation, making it analogous to the ARM Neon function `vqaddq_s64`. 
 ...
 * @return int64x2_t The result of the element-wise saturating addition, returned as an 64-bit signed integer vector.
 */
int64x2_t vqaddq_s64_rvv(int64x2_t a, int64x2_t b) {
    return __riscv_vsadd_vv_i64m1(a, b, VLEN_2);
}
