...
/**
 * @brief Performs a widening element-wise addition of an 32-bit signed integer vector to a 64-bit signed integer vector.
 * 
 * This function adds an 32-bit signed integer vector (`b`) to a 64-bit signed integer vector (`a`), where the elements
 * of the 32-bit vector are sign-extended (widened) to 64-bit before performing the addition. It utilizes the ARM NEON
 * intrinsic `vaddw_s32` to carry out the operation. The result is a new vector where each 64-bit element in the input 
...
int64x2_t vaddw_s32_rvv(int64x2_t a, int32x2_t b) {
    return __riscv_vadd_vv_i64m1(a , __riscv_vlmul_trunc_v_i64m2_i64m1(__riscv_vwadd_vx_i64m2(b, 0, VLEN_2)), VLEN_2);
}
