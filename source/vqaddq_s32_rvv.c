...
/**
 * @brief Performs an element-wise signed saturating addition of two 128-bit vectors of 32-bit signed integers.
 * 
 * This function performs an element-wise addition of two 32-bit signed integer vectors (`a` and `b`), 
 * each containing four elements. If the result of the addition exceeds the range of an 32-bit signed integer 
 * (-2147483648 to 2147483647), the result is saturated to the nearest boundary (either -2147483648 or 2147483647). It utilizes 
 * the RISC-V Vector Extension to carry out the operation, making it analogous to the ARM Neon function `vqaddq_s32`. 
 ...
 * @return int32x4_t The result of the element-wise saturating addition, returned as an 32-bit signed integer vector.
 */
int32x4_t vqaddq_s32_rvv(int32x4_t a, int32x4_t b) {
    return __riscv_vsadd_vv_i32m1(a, b, VLEN_4);
}
