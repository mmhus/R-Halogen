...
/**
 * @brief Performs an element-wise unsigned saturating addition of two 128-bit vectors of 64-bit unsigned integers.
 * 
 * This function performs an element-wise addition of two 64-bit unsigned integer vectors (`a` and `b`), 
 * each containing two elements. If the result of the addition exceeds the range of an 64-bit unsigned integer 
 * (0 to 18446744073709551615), the result is saturated to the nearest boundary (either 0 or 18446744073709551615). It 
 * utilizes the RISC-V Vector Extension to carry out the operation, making it analogous to the ARM Neon function `vqaddq_u64`. 
...
uint64x2_t vqaddq_u64_rvv(uint64x2_t a, uint64x2_t b) {
    return __riscv_vsaddu_vv_u64m1(a, b, VLEN_2);
}
